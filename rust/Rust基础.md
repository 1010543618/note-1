...
* Rust 中对于整数字面量, 默认视为 `i32` 类型, 即有符号的 32 位整型, 除此之外还有 `u32`, `i64` 等类型, 即无符号 32 位整型, 有符号 64 位整型, 如果希望给字面量指定其他类型, 可以这样 `5u8`, `6i64`
...
* 引入相对路径时, 书上说要加 `self`, 不过现在似乎不需要加 `self` 也 OK

* 引入一个模块下子模块的内容, 不会导致同时引入了该模块的内容
...
* `for...in` 迭代的时候是否可以获取索引? 使用 `Range` 见 `for` 循环章节, 或者迭代器返回元组解构得到索引